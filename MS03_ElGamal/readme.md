...
现在，Alice 希望加密消息 $M = 5$。Alice 随机选择 $k = 7$（注意要跟 $p-1$ 互质），并计算：
...
与ElGamal加密算法相似，ElGamal签名算法也使用了离散对数问题的困难性来保证签名的安全性。算法主要分为密钥生成、签名生成和签名验证三个步骤，我们用Alice（签名方）和Bob（验证方）做演示。
...
密钥生成步骤和ElGamal 加密算法相同，最终的公钥为$(p, g, y)$，公开的；私钥为$x$，隐私的。
...
Bob可以利用公开的信息 $(g, p, r, s, M)$ 来验证签名是否真实。
...
$g^{H(M)} \equiv y^rr^s \pmod{p}$ 计算比较复杂，我们可以直接验证 $xr+ks = H(M) \mod 12$ 是否成立。 $xr+ks = 4 \times 7 + 7 \times 7 = 44 = 5 = H(M) \mod 12$，因此签名有效。

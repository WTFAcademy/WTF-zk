...
5. 当 $k$ 与 $n$ 互质时，即 $\gcd(k,n)=1$ 时，如果有 $k \cdot a \equiv k \cdot b \pmod{n}$，则有 $ a \equiv b \pmod{n}$。
    
    例子：$8 \equiv 14 \pmod{3}$，由于 $\gcd(2,3)=1$，有 $4 \equiv 7 \pmod{3}$

6. 如果 $a \equiv b \pmod{n}$ 且 $c \equiv d \pmod{n}$，则有 $ac \equiv bd \pmod{n}$

...
在数论基础中，我们介绍了欧拉定理，将欧拉函数和整数模n乘法群的循环性质联系起来。

**欧拉定理：** 如果整数 $a$ 和正整数 $n$ 互质（即 $\gcd(a,n)=1$），那么 $a^{\phi(n)} \equiv 1 \pmod{n}$。

现在我们利用循环群的性质证明它。

首先考虑整数模n乘法群 $Z^*_n$，它的阶是 $\phi(n)$。设整数 $a$ 与 $n$ 互质，即 $\gcd(a,n)= 1$，有 $a \equiv 1 \mod n$， 因此 $a \in Z^*_n$。我们可以以 $a$ 为生成元构建循环群 $A$，则 $A$ 为 $Z^*_n$ 的子群。设群 $A$ 的阶为 $k$，有 $a^k \equiv 1 \pmod n$。根据拉格朗日定理，有子群 $A$ 的阶整除 $Z^*_n$ 的阶，即 $k | \phi(n)$。也就是说有整数 $q$ 使得 $\phi(n) = kq$。我们得到：
